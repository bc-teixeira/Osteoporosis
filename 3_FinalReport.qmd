...
p <- ggplot (FracTotRate, aes(x = FaixaEtr, y=IndFrac, group = DIAG_CAP,
  ...
  labs(title = "Hospitalization Rate (per 100,000 inhabitants) by type of Fracture and Age Group (2022)", x = "Age Group", y = "Hospitalization Rate (per 100,000 inhabitants)", caption = "Source: SIH -DATASUS, IBGE, ANS") +
  theme_bw() +
  theme(axis.text.x = element_text(angle = 20, hjust = 1))+
  scale_colour_discrete(guide = FALSE) +
...
```{r}
#| label: total-fracture-rate-detailed
#|fig-width: 10
...
```{r}
#| label: female-fracture-detailed
p <- p + geom_area(data = FracTotRate72, aes(x = FaixaEtr, y=n, group = DS_CID_en, fill = DS_CID_en), position = "stack", stat = "identity", alpha = 0.5) +labs(fill = "Femoral Fracture Type")
...
```{r}
#| label: female-fracture-rate-detailed
#|fig-width: 10
p <- p + geom_area(data = FracTotRate72, aes(x = FaixaEtr, y=IndFrac, group = DS_CID_en, fill = DS_CID_en), position = "stack", stat = "identity", alpha = 0.5) +labs(fill = "Femoral Fracture Type")
...
```{r}
#| label: male-fracture-rate-detailed
#|fig-width: 10
p <- p + geom_area(data = FracTotRate72, aes(x = FaixaEtr, y=IndFrac, group = DS_CID_en, fill = DS_CID_en), position = "stack", stat = "identity", alpha = 0.5) +labs(fill = "Femoral Fracture Type")
...
p <- densano%>% 
  ggplot(aes(x = ano, y = nDens)) +
        ...
        geom_point(size = 3)+
        theme_bw() + 
  #y axis to be shown in not in scientific notation
...
```{r}
#| label: densitometries-forecast-data
PA %>% 
  group_by(ano) %>% 
  summarise(nDens = sum(PA_QTDPRO), .groups = "keep")
...
```{r}
#| label: densitometries-forecast-year
...
```{r}
#| label: hip-fractures-forecast-year
...
```{r}
#| label: hip-fractures-forecast-rate-data
fracano %>% 
  mutate('80% Confience Interval' = if_else(is.na(nFracPred), NA_character_, paste(round(nFracPredlo80,0), " - ", round(nFracPredhi80,0))),
         '95% Confience Interval' = if_else(is.na(nFracPred), NA_character_, paste(round(nFracPredlo95,0), " - ", round(nFracPredhi95,0)))) %>% 
    rename(Ano = ano, 
...
```{r}
#| label: hip-fractures-bygender-number
...
```{r}
#| label: hip-fractures-bygender-rate
P <- df_fracrate %>% 
...
```{r}
 ...
 df_fracrate %>% 
    pivot_longer(cols = c(nFrac, PopSUS, IndFrac), names_to = "variable", values_to = "value") %>% 
    pivot_wider(names_from = ano, values_from = value) %>% 
    mutate(across(where(is.numeric), ~prettyNum(.x, big.mark = ",", scientific = FALSE, trim = TRUE, )))
```
...
```{r}
#| label: hip-fractures-bygender-rates-plot
...
FracRate <- ggplot(FracTotRateSexo, aes(x = FaixaEtr, y = if_else(SEXO == "Male", -FracRate,FracRate), fill = SEXO)) + 
  ...
  scale_y_continuous(labels = abs, limits = max(FracTotRateSexo$FracRate) * c(-1,1))  + 
  coord_flip() + 
  scale_fill_brewer(palette = "Set1") + 
  theme_bw() +
  labs(x = "Age Group", y = "Hip Fracture Rate per 100,000 population", title = "Hip Fracture Rate by Age Group and Sex. Brazil 2022",
...
```{r}
#| label: hip-fractures-bygender-median-age
RD %>% 
  filter(UNICO & CD_CID %in% c("S720","S721","S722")) %>% 
  group_by(SEXO) %>%
  summarise(median_age = median(IDADE),
            perc25 = quantile(IDADE, probs = 0.25),
            perc75 = quantile(IDADE, probs = 0.75))
```

```{r}
#| label: hip-fractures-bygender-rate-age-groups
FracTotRateSexo %>% 
  select(FaixaEtr, SEXO, FracRate) %>% 
  pivot_wider(names_from = SEXO, values_from = FracRate) %>%
...
Fracture rate in people over 70 years old

```{r}
#| label: hip-fractures-bygender-rate-table
#Fracture ratio for men and women over 70 years old grouped by gender
FracTotRateSexo %>% 
  filter(FaixaEtr %in% c("70 to 74 years", "75 to 79 years", "80 years or more")) %>% 
  group_by(SEXO) %>%
  summarize(n = sum(n), PopSUS = sum(PopSUS), .groups = "keep") %>%
  mutate(FracRate = n*100000/PopSUS)

```

```{r}
#| label: hip-fractures-odds_r-table
...
```{r}
#| label: hip-fractures-odds_r-plot
P <- FracRateOR %>% 
  ...
  labs(y = "Odds Ratio in relation to the previous age group",
        x = "Age Range",
        title = "Odds Ratio of hip fractures in men and women",
        subtitle = "Brazil, 2022", color = "Sex")
...
```{r}
#| label: hip-fractures-odds_r-plot-previous-age
P <- FracRateOR %>% 
  ...
  geom_hline(yintercept = 1, linetype = "dashed", alpha = 0.5) +
  #add labels to the points
  geom_text(position = position_dodge(width = 0.1), hjust = -0.3, size = 4, show.legend = FALSE) +
  #x axis labels in 45 degres
  theme(axis.text.x = element_text(angle = 45, hjust = 1, size = 12)) +
  #yaxis in log scale
  scale_y_log10() +
  #extend the limits of the x axis the text is not cut
  scale_x_discrete(expand = c(0.05, 0.12)) +
...
```{r}
#| label: densitometry-gender-age-group-plot
...
DensRate <- ggplot(DensTotRateSexo, aes(x = FaixaEtr, y = if_else(SEXO == "Male", -DensRate,DensRate), fill = SEXO)) + 
  ...
  scale_y_continuous(labels = abs, limits = max(DensTotRateSexo$DensRate) * c(-1,1))  + 
  coord_flip() + 
  scale_fill_brewer(palette = "Set1") + 
  theme_bw() +
  labs(x = "Age Group", y = "Bone Densitometry Rate per 100,000 inhabitants", title = "Bone Densitometry Rate by Age Group and Sex",
...
**Densitometry rate in men and women over 70 years old**
```{r}
#| label: densitometry-gender-over70-table
# Densitometry rate in men and women over 70 years old
DensTotRateSexo %>% 
  filter(FaixaEtr %in% c("60 to 64 years", "65 to 69 years","70 to 74 years", "75 to 79 years", "80 years or more")) %>% 
  group_by(SEXO) %>%
  summarize(n = sum(n), PopSUS = sum(PopSUS), .groups = "keep") %>%
  mutate(DensRate = n*100000/PopSUS)
```
...
```{r}
#| label: dens-frac-rates-disparity
...
**Densitometry numbers >50**
```{r}
#| label: dens-frac-rates-disparity_table
dens_rate_raw %>% 
  mutate(SEXO = droplevels(SEXO)) %>%
  pivot_longer(cols = c(n, PopSUS, DensRate), names_to = "variable", values_to = "value") %>% 
  pivot_wider(names_from = ano, values_from = value) %>% 
  mutate(across(where(is.numeric), ~prettyNum(.x, big.mark = ",", scientific = FALSE, trim = TRUE, )))
```

**Densitometry Numbers for Male >70 and Female >65**
```{r}
#| label: dens-numbers_segmented
dens_rate_raw_segmented <- PA %>% 
  filter(SEXO != "Undefined") %>% 
  group_by(FaixaEtr, SEXO, ano) %>%
  summarize(n = sum(PA_QTDPRO), .groups = "keep") %>% 
  left_join(popSUSBR %>% group_by(FaixaEtr, Sexo, ano) %>% summarise(PopSUS = sum(PopSUS), .groups = "keep"), by = c("FaixaEtr", "SEXO" = "Sexo", "ano")) %>% 
  filter((SEXO == "Female" & as.numeric(word(FaixaEtr))>=65) | (SEXO == "Male" & as.numeric(word(FaixaEtr))>=70)) %>% 
  group_by(SEXO, ano) %>% 
  summarise(n = sum(n),
            PopSUS = sum(PopSUS), .groups = "keep") %>% 
  mutate(DensRate = n*100000/PopSUS)

dens_rate_raw_segmented %>% 
  mutate(SEXO = droplevels(SEXO)) %>%
  pivot_longer(cols = c(n, PopSUS, DensRate), names_to = "variable", values_to = "value") %>% 
  pivot_wider(names_from = ano, values_from = value) %>% 
  mutate(across(where(is.numeric), ~prettyNum(.x, big.mark = ",", scientific = FALSE, trim = TRUE, )))
...
```{r}
#| label: dx-scanners-rate
dx_rate <- EQ %>% 
  group_by(year = year(DATA), SUS) %>% 
  summarise(QT = sum(QT),
            QT_USO = sum(QT_USO), .groups = "keep") %>% 
  left_join(popSUSBR %>% group_by(ano) %>% filter(word(FaixaEtr,1) >= 50) %>%
              summarise(SUS = sum(PopSUS), `Non-SUS` = sum(PopANS), .groups = "keep") %>% 
...
## Table with procedures codes
Number of hip fractures in SUS:
```{r}
#| label: raw-table-hip-frac
#Generate table with procedures codes used in the Hip Fracture analysis RD
#Filtering only the procedures used in the analysis


#Annual Fracture rate by gender in the whole time period in RD with the SUS population in the same period 
RD %>% 
  filter(UNICO & CD_CID %in% c("S720","S721","S722")) %>% 
  filter(IDADE >= 50) %>% 
  group_by(ano, SEXO) %>% 
...
Median of age in hip fracture hospitalisations:
```{r}
#| label: raw-table-hip-frac-age
#Median of age in hip fracture hospitalisations
RD %>% 
  filter(UNICO & CD_CID %in% c("S720","S721","S722")) %>% 
  filter(IDADE >= 50) %>% 
  group_by(SEXO) %>% 
  summarise(median_age = median(IDADE), .groups = "keep", 
            percentile25 = quantile(IDADE, 0.25),
            percentile75 = quantile(IDADE, 0.75))
...
```{r}
#| label: adjusted-frac-rate
FracStdRate <- RD %>% 
  filter(UNICO & CD_CID %in% c("S720","S721","S722")) %>% 
  filter(ano >= 2008) %>% 
  filter(IDADE >= 50) %>% 
  group_by(ano, SEXO, FaixaEtr) %>% 
  summarise(nFrac = n_distinct(N_AIH), .groups = "keep") %>% 
  left_join(popSUSBR %>% group_by(ano, Sexo, FaixaEtr) %>% filter(word(FaixaEtr,1) >= 50) %>% summarise(PopSUS = sum(PopSUS), .groups = "keep"), by = c("ano", "SEXO" = "Sexo", "FaixaEtr")) %>% 
  group_by(ano, FaixaEtr) %>%
  summarise(PopSUS = sum(PopSUS),
            nFrac = sum(nFrac), .groups = "keep") %>%
  left_join(std_WHO, by = "FaixaEtr") %>%
  mutate(act_rate = nFrac/PopSUS,
         WHO_n = act_rate * WHO20002025) %>%
  group_by(ano) %>%
  summarise(PopSUS = sum(PopSUS),
            nFrac = sum(nFrac),
            WHO20002025 = sum(WHO20002025),
            WHO_n = sum(WHO_n)) %>% 
  mutate(std_rate = WHO_n*100000/WHO20002025,
         act_rate = nFrac*100000/PopSUS)

FracStdRate %>%
  ggplot(aes(x = ano)) +
  geom_line(aes(y = std_rate, color = "Standardised Rate"), linewidth = 1) + 
  geom_line(aes(y = act_rate, color = "Actual Rate"), linewidth = 1) +
  theme(axis.text.x = element_text(angle = 20, hjust = 1)) +
labs(caption = "Source: SIH-SUS, IBGE, ANS",
     subtitle = "Considering Population over 50 years old", 
     title = "Standardised vs Actual Hip Fracture Rates - Brazil, 2008-2022",
     x = "Year", y = "Rate per 100,000", color = "Rate Type") +
theme_bw() +
scale_y_continuous(limits = c(0, NA))
```

```{r}
#| label: adjusted-frac-rate-table
#Create a piovt table of FracStdRate with ano as column
FracStdRate %>%
pivot_longer(col= -ano) %>%
pivot_wider(names_from = ano, values_from = value) %>% 
mutate(across(everything(), ~formatC(., format = "f", digits = 0)))

```

The above plot and table, describes the difference between the actual and standardised rates of hip fractures in Brazil from 2008 to 2022. The actual rate is the number of hip fractures per 100,000 people aged 50 and over in Brazil. The standardised rate is the number of hip fractures per 100,000 people aged 50 and over in Brazil, adjusted for the age distribution of the world population in 2000-2025. The actual rate is the same as the standardised rate in 2008, but it increases in the following years. The actual rate is `r format(FracStdRate %>% filter(ano == 2022) %>% pull(act_rate), format = "f", digits = 4)` hip fractures per 100,000 people aged 50 and over in Brazil in 2022. The standardised rate is `r format(FracStdRate %>% filter(ano == 2022) %>% pull(std_rate), format = "f", digits = 4)` hip fractures per 100,000 people aged 50 and over in Brazil in 2022. This means that the aging of population is only responsible for a small part of the increase in hip fractures in Brazil.

Of the `r format(((FracStdRate %>% filter(ano == 2022) %>% pull(act_rate)/FracStdRate %>% filter(ano == 2008) %>% pull(act_rate))-1)*100,digits = 3)`% growth in hip fractures rate from 2008 to 2022, only `r format(((FracStdRate %>% filter(ano == 2022) %>% pull(act_rate)/FracStdRate %>% filter(ano == 2008) %>% pull(act_rate))/(FracStdRate %>% filter(ano == 2022) %>% pull(std_rate)/FracStdRate %>% filter(ano == 2008) %>% pull(std_rate))-1)*100,digits = 3)`% of the growth can be explained by the aging of the population.