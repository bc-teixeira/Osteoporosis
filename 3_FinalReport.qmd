...
To identify procedures that would uniquely identify a fracture, we have used the folowing list of procedures codes:
...
```{r}
#|fig-width: 10
FracTotRate <- RD %>% filter (year(DT_INTER) == 2022, SEXO == "Feminino") %>% 
  group_by(FaixaEtr, DIAG_CAP, DS_CID_CAP) %>%
  summarize(n = n_distinct(N_AIH), .groups = "keep") %>% 
  left_join(popSUSBR %>% filter(ano == 2022) %>% group_by(FaixaEtr) %>% summarise(PopSUS = sum(PopSUS)), by = "FaixaEtr") %>%
  mutate(IndFrac = n*100000/PopSUS)

FracTotRate72 <- RD %>% filter (year(DT_INTER) == 2022, SEXO == "Feminino") %>%  filter (DIAG_CAP =="S72")%>%
  group_by(FaixaEtr, DS_CID, DIAG_CAP) %>% summarize(n = n_distinct(N_AIH), .groups = "keep") %>% 
  left_join(popSUSBR %>% filter(ano == 2022) %>% group_by(FaixaEtr) %>% summarise(PopSUS = sum(PopSUS)), by = "FaixaEtr") %>%
  mutate(IndFrac = n*100000/PopSUS)
...
```{r}
#|fig-width: 10
FracTotRate <- RD %>% filter (year(DT_INTER) == 2022, SEXO == "Masculino") %>% 
  group_by(FaixaEtr, DIAG_CAP, DS_CID_CAP) %>%
  summarize(n = n_distinct(N_AIH), .groups = "keep") %>% 
  left_join(popSUSBR %>% filter(ano == 2022) %>% group_by(FaixaEtr) %>% summarise(PopSUS = sum(PopSUS)), by = "FaixaEtr") %>%
  mutate(IndFrac = n*100000/PopSUS)

FracTotRate72 <- RD %>% filter (year(DT_INTER) == 2022, SEXO == "Masculino") %>%  filter (DIAG_CAP =="S72")%>%
  group_by(FaixaEtr, DS_CID, DIAG_CAP) %>% summarize(n = n_distinct(N_AIH), .groups = "keep") %>% 
  left_join(popSUSBR %>% filter(ano == 2022) %>% group_by(FaixaEtr) %>% summarise(PopSUS = sum(PopSUS)), by = "FaixaEtr") %>%
  mutate(IndFrac = n*100000/PopSUS)
...
RD %>% 
  ...
  ggplot(aes(x = ano, y = nFrac, color = SEXO)) +
        ...
        labs(x = "Ano", y = "Número de Fraturas", title = )+
...
FracRateOR %>% 
  ...
  #yaxis in log scale
  scale_y_log10() +
  labs(y = "Odds Ratio em relação ao grupo etário anterior",
       x = "Faixa Etária",
       title = "Odds Ratio de fraturas de fêmur em homens e mulheres",
       subtitle = "Brasil, 2020")
...
FracRateOR %>% 
  ...
  #yaxis in log scale
  scale_y_log10() +
  labs(y = "Odds Ratio em relação às idades anteriores",
       x = "Faixa Etária",
       title = "Odds Ratio de fraturas de fêmur em homens e mulheres",
       subtitle = "Brasil, 2020")
...
DensRate <- ggplot(DensTotRateSexo, aes(x = FaixaEtr, y = if_else(SEXO == "Masculino", -IndDens,IndDens), fill = SEXO)) + 
  ...
  labs(x = "Age Group", y = "Bone Densitometry Index per 100,000 inhabitants", title = "Bone Densitometry Index by Age Group and Sex",
        caption = "Source: IBGE, ANS, SIA-SUS (2022)", fill = "Gender")
...
DensRateSex <- PA %>% 
  ...
  mutate(MaleRateDens = Masculino/Feminino)
...
FracRateSex <-RD %>% 
  ...
  mutate(MaleRateFrac = Masculino/Feminino)
